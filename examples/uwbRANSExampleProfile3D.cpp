/** @file uwbRANSExampleProfile3D.cpp

Author(s): H. Hornikova, E. Turnerova, K. Michalkova
*/
...
template<class TT>
gsMultiPatch<TT> BSplineProfile2DBetweenPatch(TT const & index,
    TT const & length_x1,
    TT const & length_x2,
    TT const & pitch,
    TT const & camberX,
...
template<class T> void refineBasisUniformZones(gsMultiBasis<T>& tbasis, int numRefineUniformLocal_v, int numUniformKnot_v); //in v direction
template<class T> void refineBasisZones(gsMultiBasis<T>& tbasis, int numRefineLocal_v, int numKnot_v, int numRefineLocal_u0, int numRefineLocal_u1_s, int numRefineLocal_u1_e, int numRefineLocal_u2, int numKnot_u0, int numKnot_u1_s, int numKnot_u1_e, int numKnot_u2);
template<class T> void refineBasisZonesBlade(gsTensorBSpline<2,T>& blade, int numRefineLocal_u1_s, int numRefineLocal_u1_e,  int numKnot_u1_s, int numKnot_u1_e);
...
int main(int argc, char *argv[])
{
    ...
    real_t weightEfficiency = 0.5;
    real_t weightOutVel = 0.5;
    real_t velObjPart = 0;
    ...
    //-----------------------------------------------
    //choose k-omega turbulence model: koWilcoxLRN (LRN Wilcox 2006) or koSST (k-omega SST; Poisson equation needs to be solved to approximate wall distance)
    ...
    //=========================================== set refinement =====================================================
    int numRefine, numRefineUniformLocal_v, numUniformKnot_v, numRefineLocal_v, numKnot_v, numRefineLocal_u0, numKnot_u0,
        numRefineLocal_u1_s, numKnot_u1_s, numRefineLocal_u1_e, numKnot_u1_e, numRefineLocal_u2, numKnot_u2, numRefineLocalFirstKnot_v;
    ...
    switch (refinement) {
      case 1: //~7080 dofs
        ...
        numRefineLocal_u0 = 4;
        numKnot_u0 = 1;
        numRefineLocal_u1_s = 4;
        numKnot_u1_s = 1;
        numRefineLocal_u1_e = 0;
        numKnot_u1_e = 1;
        numRefineLocal_u2 = 0;
        numKnot_u2 = 1;

        numRefineLocalFirstKnot_v = 0; //after refinement one more time from the first knot (refined net) in v

      break;

      case 2: //~17000 dofs
        numRefine = 1; //geometry_between = true is assumed
      ...
      default: //default uniform refinement
        ...
        //no local refinement
        numRefineUniformLocal_v = 0; numUniformKnot_v = 1; numRefineLocal_v = 0; numKnot_v = 2; numRefineLocal_u0 = 0;
        numKnot_u0 = 1; numRefineLocal_u1_s = 0; numKnot_u1_s = 1; numRefineLocal_u1_e = 0; numKnot_u1_e = 1;
        numRefineLocal_u2 = 0; numKnot_u2 = 1; numRefineLocalFirstKnot_v = 0;
   ...
   std::vector<gsBSpline<>> defcurves_suctionside_pSol;
   std::vector<gsBSpline<>> defcurves_pressureside_pSol;
   std::vector<gsBSpline<>> defcurves_pIn;
   std::vector<gsBSpline<>> defcurves_pOut;
   std::vector<gsBSpline<>> defcurves_velIn;
   std::vector<gsBSpline<>> defcurves_velOut;
...
*/
    ...
    for(unsigned i_blade=0; i_blade < num_bladeprofiles; i_blade++) //to compute profiles 0-6
    {
    gsInfo << "===================================================\n";
    file << "\n===================================================\n";
    index_of_profile = i_blade;
    ...
    gsMultiPatch<real_t> patches;
      gsMultiPatch<real_t> patches_old;
    if (geometry_between)
    {
        patches_old = BSplineProfile2DBetweenPatch<real_t>(index_of_profile,
        ...
        patches_old.addInterface(0, boundary::north,(size_t) 0, boundary::south);
        patches_old.addInterface(2, boundary::north, 2, boundary::south);
    }
    else
    {
    ...
    //patches = makeLinear(patches_old,1);
    patches = patches_old;
    ...
    if (!loadIC)
    {
        ...
        gsField<> pressureSteady = navStokes.constructSolution(1);
    ...
    if (loadIC)
    {
        gsFileData<> fdReadTM("profile"+ profile +"_TM_solution.xml");
        TMSolution = *(fdReadTM.getFirst< gsMatrix<real_t> >());
        turbSolver_unsteady.setInitialCondition(TMSolution);
    }
    else
        turbSolver_unsteady.setInitialCondition(turbSolver.getSolution());
    //if SST turbulence model is chosen, Poisson equation must be solved to approximate wall distance of the interior points
    ...
    if (loadIC)
    {
        gsFileData<> fdRead("profile"+ profile +"_RANS_solution.xml");
        RANSSolution = *(fdRead.getFirst< gsMatrix<real_t> >());
    ...
    objFunction.initialize(profilePatchNumbers, profileSides, normalStream, rr[index_of_profile],
        outPatchNumbers, outSides, uTangentialTarget, velAvg);
    // sum of the weights must equal to 1!!!
    objFunction.setWeights(0.0, weightOutVel, 0.0, weightEfficiency);
    ...
    objFunction.setTargetPressureAtProfile(-0.15, 0.15);
    velObjPart += objFunction.evaluate();
    gsVector<real_t> objectiveParts = objFunction.getObjectives();
    std::ofstream fileObjective;
    fileObjective.open("InitialObjectiveParts"+profile+".txt");
    ...
    gsInfo << "\nAverage tangential velocity = " << objFunction.evaluateOutflowVelocityAvg(0, 1) << "\n";
    gsInfo << "Target tangential velocity = " << uTangentialTarget << "\n";
    gsInfo << "Average meridial velocity = " << objFunction.evaluateOutflowVelocityAvg(0, 0) << "\n\n";
    file << "\nAverage tangential velocity = " << objFunction.evaluateOutflowVelocityAvg(0, 1) << "\n";
    file << "Target tangential velocity = " << uTangentialTarget << "\n";
    file << "Average meridial velocity = " << objFunction.evaluateOutflowVelocityAvg(0, 0) << "\n\n";
    ...
    /*real_t prutokIn = ransSolver.getAssembler()->getBlockAssembler().computeFlowRate(0, boundary::west, ransSolver.getSolution());
    real_t prutokOut = ransSolver.getAssembler()->getBlockAssembler().computeFlowRate(2, boundary::east, ransSolver.getSolution());
    gsInfo << "prutokIn" << prutokIn << "\n";
    gsInfo << "prutokOut" << prutokOut << "\n";
    system("pause");*/
 
    //======save pressure of blade for 3D with loft=========
    const gsTensorBSplineBasis<2, real_t>*  basisBladePatch = dynamic_cast<const gsTensorBSplineBasis<2, real_t>*>(&(tbasis.basis(1)));
    gsBSpline<> pSol_spline_pressureSide(basisBladePatch->knots(0),pSolution.coefficientVector(1).block(0,0,tbasis.at(1).component(0).size(),1));
    gsBSpline<> pSol_spline_suctionSide(basisBladePatch->knots(0),pSolution.coefficientVector(1).block(tbasis.at(1).size()-tbasis.at(1).component(0).size()-1,0,tbasis.at(1).component(0).size(),1));

    defcurves_suctionside_pSol.push_back(pSol_spline_suctionSide);
    defcurves_pressureside_pSol.push_back(pSol_spline_pressureSide);

    //======save pressure at the inlet for 3D with loft=========
    int patchInlet_vSize = tbasis.at(0).component(1).size();
    int patchInlet_uSize = tbasis.at(0).component(0).size();


    gsMatrix<> pIn_coefs(patchInlet_vSize,1);
    int vValue_index = 0;
    for(int i = 0; i < pIn_coefs.rows(); i++)
    {
        pIn_coefs(i,0) =  (pSolution.coefficientVector(0)).at(vValue_index);
        vValue_index += patchInlet_uSize;
    }


    const gsTensorBSplineBasis<2, real_t>*  basisInputPatch = dynamic_cast<const gsTensorBSplineBasis<2, real_t>*>(&(tbasis.basis(0)));
    gsBSpline<> pIn_spline(basisInputPatch->knots(1), pIn_coefs);

    defcurves_pIn.push_back(pIn_spline);

    //======save pressure at the Outlet for 3D with loft=========
    int patchOutlet_vSize = tbasis.at(2).component(1).size();
    int patchOutlet_uSize = tbasis.at(2).component(0).size();


    gsMatrix<> pOut_coefs(patchOutlet_vSize,1);
    vValue_index = patchOutlet_uSize - 1;
    for(int i = 0; i < pOut_coefs.rows(); i++)
    {
        pOut_coefs(i,0) =  (pSolution.coefficientVector(2)).at(vValue_index);
        vValue_index += patchOutlet_uSize;
    }

    gsBSpline<> pOut_spline(basisInputPatch->knots(1), pOut_coefs);
    defcurves_pOut.push_back(pOut_spline);


    //======save velocity at the inlet for 3D with loft=========
    int patchInlet_vSize_vel = discreteBases[0].at(0).component(1).size();
    int patchInlet_uSize_vel = discreteBases[0].at(0).component(0).size();


    gsMatrix<> velIn_coefs(patchInlet_vSize_vel,2);
    vValue_index = 0;
    for(int i = 0; i < velIn_coefs.rows(); i++)
    {
        velIn_coefs.row(i) =  (uSolution.coefficientVector(0)).row(vValue_index);
        vValue_index += patchInlet_uSize_vel;
    }



    const gsTensorBSplineBasis<2, real_t>*  basisInputPatch_vel = dynamic_cast<const gsTensorBSplineBasis<2, real_t>*>(&(discreteBases[0].basis(0)));
    gsBSpline<> velIn_spline(basisInputPatch_vel->knots(1), velIn_coefs);

    defcurves_velIn.push_back(velIn_spline);

    //======save pressure at the Outlet for 3D with loft=========
    int patchOutlet_vSize_vel = discreteBases[0].at(2).component(1).size();
    int patchOutlet_uSize_vel = discreteBases[0].at(2).component(0).size();


    gsMatrix<> velOut_coefs(patchOutlet_vSize_vel,2);
    vValue_index = patchOutlet_uSize_vel - 1;
    for(int i = 0; i < velOut_coefs.rows(); i++)
    {
        velOut_coefs.row(i) =  (uSolution.coefficientVector(2)).row(vValue_index);
        vValue_index += patchOutlet_uSize_vel;
    }


    const gsTensorBSplineBasis<2, real_t>*  basisOutputPatch_vel = dynamic_cast<const gsTensorBSplineBasis<2, real_t>*>(&(discreteBases[0].basis(2)));
    gsBSpline<> velOut_spline(basisOutputPatch_vel->knots(1), velOut_coefs);

    defcurves_velOut.push_back(velOut_spline);


    } //end of for 1-6 profile

        //======pressure 3D=========
        gsKnotVector<> kvloft(0, 1, num_bladeprofiles-4, 4);
        gsKnotVector<> kvloft_vel(0, 1, num_bladeprofiles-5, 5);

        //-----pressure blade-------
        gsTensorBSpline<2,real_t> suction_side_surface_pSol;
        gsTensorBSpline<2,real_t> pressure_side_surface_pSol;
        gsTensorBSpline<2,real_t> suction_side_surface_blade3D;
        gsTensorBSpline<2,real_t> pressure_side_surface_blade3D;


        gsMatrix<> section_parameters(1,num_bladeprofiles);
        for (unsigned i = 0; i < num_bladeprofiles; i++) {
        section_parameters(0,i) = (rr[i]-rr[0])/(rr[num_bladeprofiles-1]-rr[0]);
        }
        //gsInfo << section_parameters << "\n";

        computeLoftFuction(defcurves_suctionside_pSol,  defcurves_suctionside_pSol[0].basis().knots(), num_bladeprofiles, kvloft, section_parameters, suction_side_surface_pSol, 1);
        computeLoftFuction(defcurves_pressureside_pSol,  defcurves_pressureside_pSol[0].basis().knots(), num_bladeprofiles, kvloft, section_parameters, pressure_side_surface_pSol, 1);
        ...
        KaplanTurbineRunnerBlade<real_t> runnerBlade(num_bladeprofiles, camber_x, camber_y, leading_angle, trailing_angle, thickness_x, thickness_y, ending_offset,
                                     output_angle, radius, chord_length, angle, rotation_center_x, rotation_center_y, rr, extrapolateBlade);
        ...
        gsField<> pSol_3D_suction(suction_side_surface_blade3D, suction_side_surface_pSol);
        gsField<> pSol_3D_pressure(pressure_side_surface_blade3D, pressure_side_surface_pSol);
       ...
       gsFileData<> file_inputSurface(MOTOR_DATA_DIR "uwb-pilsen/fileSurfaceIn.xml");

       gsGeometry<>::uPtr inputSurface;
       if (file_inputSurface.has< gsGeometry<> >())
       {
           inputSurface = file_inputSurface.getFirst< gsGeometry<> >();
       }
       else
       {
       ...
       gsTensorBSpline<2,real_t> surface_pIn;
       computeLoftFuction(defcurves_pIn,  defcurves_pIn[0].basis().knots(), num_bladeprofiles, kvloft, section_parameters, surface_pIn, 1);

       gsField<> pIn_3D(*inputSurface, surface_pIn);


       gsGeometry<>::uPtr outputSurface;
       gsFileData<> file_outputSurface(MOTOR_DATA_DIR "uwb-pilsen/fileSurfaceOut.xml");

       if (file_outputSurface.has< gsGeometry<> >())
       {
           outputSurface = file_outputSurface.getFirst< gsGeometry<> >();
       }
       else
       {
           gsWarn << "Input file doesn't have a geometry.\n";
       ...
       gsTensorBSpline<2,real_t> surface_pOut;
       computeLoftFuction(defcurves_pOut,  defcurves_pOut[0].basis().knots(), num_bladeprofiles, kvloft, section_parameters, surface_pOut,  1);


       gsField<> pOut_3D(*outputSurface, surface_pOut);



       //velocity

       /*gsTensorBSpline<2,real_t> surface_velIn;
       computeLoftFuction(defcurves_velIn,  defcurves_velIn[0].basis().knots(), num_bladeprofiles, kvloft_vel, section_parameters, surface_velIn, 2);


       gsField<> velIn_3D(*inputSurface, surface_velIn);

       gsTensorBSpline<2,real_t> surface_velOut;
       computeLoftFuction(defcurves_velOut,  defcurves_velOut[0].basis().knots(), num_bladeprofiles, kvloft_vel, section_parameters, surface_velOut,2);


       gsField<> velOut_3D(*outputSurface, surface_velOut);*/

              if (true)
       {
           gsWriteParaview<>(suction_side_surface_pSol, "blade3D_solution_suction", plot_pts);
           gsWriteParaview<>(pressure_side_surface_pSol, "blade3D_solution_pressure", plot_pts);
           gsWriteParaview(pressure_side_surface_blade3D, "blade3D_geometry_pressure",plot_pts);
           gsWriteParaview(suction_side_surface_blade3D, "blade3D_geometry_suction",plot_pts);
           gsWriteParaview(pSol_3D_pressure, "blade3D_field_pressure",plot_pts);
           gsWriteParaview(pSol_3D_suction, "blade3D_field_suction",plot_pts);
           gsWriteParaview(surface_pIn, "inputSurface_solution_pressure",plot_pts);
           gsWriteParaview(*inputSurface, "inputSurface_geometry",plot_pts);
           gsWriteParaview(pIn_3D, "inputSurface_field_pressure",plot_pts);
           gsWriteParaview(surface_pOut, "outputSurface_solution_pressure",plot_pts);
           gsWriteParaview(*outputSurface, "outputSurface_geometry",plot_pts);
           gsWriteParaview(pOut_3D, "outputSurface_field_pressure",plot_pts);

           //gsWriteParaview(velIn_3D, "inputSurface_field_velocity",plot_pts);
            //gsWriteParaview(surface_velIn, "inputSurface_solution_velocity",plot_pts);
            //gsWriteParaview(velOut_3D, "outputSurface_field_velocity",plot_pts);
            //gsWriteParaview(surface_velOut, "outputSurface_solution_velocity",plot_pts);


       }
              gsFileData<> fileBladePressure;
              fileBladePressure << pressure_side_surface_blade3D;
              fileBladePressure.save("fileBladePressure.xml");

              gsFileData<> fileBladeSuction;
              fileBladeSuction << suction_side_surface_blade3D;
              fileBladeSuction.save("fileBladeSuction.xml");


     //========================================
        const gsBasis<>& basisSuction = suction_side_surface_pSol.basis();
        const gsBasis<>& basisPressure = pressure_side_surface_pSol.basis();

        const gsBasis<>& basisInlet = surface_pIn.basis();
        const gsBasis<>& basisOutlet = surface_pOut.basis();

        //const gsBasis<>& basisUInlet = surface_velIn.basis();
        //const gsBasis<>& basisUOutlet = surface_velOut.basis();

        //real_t head = H;
        index_t numBlades = num_blades;
        real_t density = 997.;
        uwbObjectiveFunctionEvaluator<real_t> objFnEfficiency(pSol_3D_suction);
        objFnEfficiency.setEfficiencyWeight(weightEfficiency);
        objFnEfficiency.initialize(angularVelocity, flow_rate, gravity, numBlades, pSol_3D_suction.parDim(), density);
        real_t head = objFnEfficiency.evaluateHeadFromLoftedPressure(basisInlet, pIn_3D, vel_mer_in, gravity);
        head -= objFnEfficiency.evaluateHeadFromLoftedPressure(basisOutlet, pOut_3D, vel_mer_in, gravity);
        //real_t head = objFnEfficiency.evaluateHeadFromLoftedPressure(pIn_3D, basisUInlet, velIn_3D, gravity);
        //head -= objFnEfficiency.evaluateHeadFromLoftedPressure(pOut_3D, basisUOutlet, velOut_3D, gravity);
        gsInfo << "\nhead = " << head << "\n";
        file << "\nhead = " << head << "\n";
        objFnEfficiency.setHead(head);
        //--------
        //real_t initialEfficiency;
        //objFnEfficiency.setInitialEfficiency(initialEfficiency);
        //--------
        real_t objFn3D_suction = objFnEfficiency.evaluateEfficiencyFromLoftedPressure(basisSuction);
        real_t efficiency_suction = objFnEfficiency.getEfficiency();
//        gsInfo << "efficiency_suction = " << efficiency_suction << "\n";
//        file << "efficiency_suction = " << efficiency_suction << "\n";
        objFnEfficiency.updateSolution(pSol_3D_pressure);
        real_t objFn3D_pressure = objFnEfficiency.evaluateEfficiencyFromLoftedPressure(basisPressure, -1);
        real_t efficiency_pressure = objFnEfficiency.getEfficiency();
//        gsInfo << "efficiency_pressure " << efficiency_pressure << "\n";
//        file << "efficiency_pressure = " << efficiency_pressure << "\n";
        real_t objFnEfficiency3D = objFn3D_suction + objFn3D_pressure;
        gsInfo << "efficiency = " << efficiency_suction + efficiency_pressure << "\n";
        file << "efficiency = " << efficiency_suction + efficiency_pressure << "\n";

        //real_t flowRateIn = objFnEfficiency.evaluateFlowRateFromLoftedVelocity(basisUInlet, velIn_3D);
        //real_t flowRateOut = objFnEfficiency.evaluateFlowRateFromLoftedVelocity(basisUOutlet, velOut_3D);
//        gsInfo << "flowRateIn = " << flowRateIn << "\n";
//        gsInfo << "flowRateOut = " << flowRateOut << "\n";

        real_t objFn = velObjPart / num_bladeprofiles + objFnEfficiency3D;
        gsInfo << "Objective function = " << objFn << "\n";
        file << "Objective function = " << objFn << "\n";
      //========================================
...
template<class T> void refineBasisZonesBlade(gsTensorBSpline<2, T> &blade,
     int numRefineLocal_u1_s, int numRefineLocal_u1_e,  int numKnot_u1_s, int numKnot_u1_e)
{


gsTensorBSplineBasis<2,real_t> basisBlade;
basisBlade = blade.basis();
    ...
    {
        // in each refinement step take the first numKnot_v non_zero knots
        const gsTensorBSplineBasis<2, real_t>*  basis_u1_s = dynamic_cast<const gsTensorBSplineBasis<2, real_t>*>(&(basisBlade)); //basis of the first patch
    ...
    {
        // in each refinement step take the first numKnot_v non_zero knots
        const gsTensorBSplineBasis<2, real_t>*  basis_u1_e = dynamic_cast<const gsTensorBSplineBasis<2, real_t>*>(&(basisBlade)); //basis of the first patch
        uRefineKnot_u1_e.setZero(numKnot_u1_e + 1);
        int sizeKnots_1 = basis_u1_e->knots(0).size() - 1;
    ...
    TT const & uniformity_param)
{
   ...
   a_cp.setZero(num_cpblade + kvfit.degree() + kvfit.degree(), 2);
    ...
    real_t ystart_coor = -((cp_bs(0, 1)*cp_bs(cpk, 0) - cp_bs(0, 0)*cp_bs(cpk, 1) - cp_bs(0, 1)*length_x1 + cp_bs(cpk, 1)*length_x1) / (
    ...
    gsInfo << b_cp << "\n";
    gsInfo << "\n -------- \n";
    real_t yend_coor = -((cp_bs(0, 1)*cp_bs(cpk, 0) - cp_bs(0, 0)*cp_bs(cpk, 1) - cp_bs(0, 1)*length_x2 + cp_bs(cpk, 1)*length_x2) / (
        cp_bs(0, 0) - cp_bs(cpk, 0)));
...
template<class TT>
gsMultiPatch<TT> BSplineProfile2DBetweenPatchOld(TT const & index,
    TT const & length_x1,
    TT const & length_x2,
    TT const & pitch,
    TT const & camberX,
