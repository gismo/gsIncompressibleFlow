...
int main(int argc, char* argv[])
{
    ...
    // Note: For pressure, we typically only need one point, not a whole boundary
    // Commenting this out as it may be causing issues with pressure distribution
    ...
    // Let the solver handle pressure reference automatically - no explicit pressure constraint needed
    ...
    auto meshMotion = [&](real_t t) -> gsMatrix<real_t>
    {
        ...
        const real_t angle = maxAngle * std::sin(2 * M_PI / timeSpan * t) * M_PI / 180.0; // Convert to radians
        ...
        // Traverse all patches and all free DOFs
        // This is a cleaner and more robust approach
        for (index_t p = 0; p < fluidDomain.nPatches(); ++p)
        {
            ...
            for (index_t i = 0; i < originalCoefs.rows(); ++i)
            {
                ...
                // Check if the point lies on the boundary of the rotating square
    ...
    // For high Reynolds number, solve nonlinear steady Navier-Stokes as better initial condition
    gsInfo << "Solving steady Navier-Stokes for better initial condition (Re=" << Re << ")...\n";
    
    // Temporarily disable ALE for steady solve
    solver.setALEActive(false);
    
    // Solve steady Navier-Stokes using Picard iterations
    for (int i = 0; i < 10; ++i)
    {
        ...
        static gsMatrix<> prevVel = vel;
        static gsMatrix<> prevPres = pres;
        
        real_t velChange = (vel - prevVel).norm() / vel.norm();
        real_t presChange = (pres - prevPres).norm() / pres.norm();
        
        gsInfo << "  Steady iteration " << i << ": Max velocity = " << vel.lpNorm<gsEigen::Infinity>()
               << ", Max pressure = " << pres.lpNorm<gsEigen::Infinity>()
               << ", Relative change: velocity = " << velChange << ", pressure = " << presChange << "\n";
               
        prevVel = vel;
        prevPres = pres;
        
        if (velChange < 1e-4 && presChange < 1e-4)
        {
            gsInfo << "  Converged after " << i+1 << " iterations\n";
            break;
        }
    }
    
    // Re-enable ALE for time stepping
    solver.setALEActive(true);
    
    // Get initial solution statistics
    gsMatrix<> initVel = solver.solutionCoefs(0);
    gsMatrix<> initPres = solver.solutionCoefs(1);
    gsInfo << "Initial solution (steady Navier-Stokes): Max velocity = " << initVel.lpNorm<gsEigen::Infinity>()
           << ", Max pressure = " << initPres.lpNorm<gsEigen::Infinity>() << "\n";
    ...
    for (index_t step = 0; step <= nTimeSteps; ++step)
    {
        ...
        real_t angle = maxAngle * std::sin(2 * M_PI / timeSpan * time) * M_PI / 180.0; // Convert to radians, consistent with mesh motion
        centralSquareNew = centralSquare;
        gsNurbsCreator<>::rotate2D(centralSquareNew.patch(0), angle * 180.0 / M_PI, 0.5, 0.5); // Convert back to degrees for rotate2D
        ...
        if (step > 0)
        {
            ...
            // Check if solution is actually changing
            ...
            static gsMatrix<> prevVel = initVel;
            static gsMatrix<> prevPres = initPres;
            real_t velChange = (velCoefs - prevVel).norm() / prevVel.norm();
            real_t presChange = (presCoefs - prevPres).norm() / prevPres.norm();
            gsInfo << "  Relative change: velocity = " << velChange << ", pressure = " << presChange << "\n";
            prevVel = velCoefs;
            prevPres = presCoefs;
        }
        else
        {
            // At step 0, just output initial solution
            gsInfo << "  Initial solution (from Stokes)\n";
            gsInfo << "  Max velocity: " << initVel.lpNorm<gsEigen::Infinity>() << "\n";
            gsInfo << "  Max pressure: " << initPres.lpNorm<gsEigen::Infinity>() << "\n";
        }
        
        // Get current solution for output
        gsMatrix<> velCoefs = (step == 0) ? initVel : solver.solutionCoefs(0);
        gsMatrix<> presCoefs = (step == 0) ? initPres : solver.solutionCoefs(1);
        ...
        if (step % outputInterval == 0)
        {
            ...
            for (size_t p = 0; p < fluidDomain.nPatches(); ++p)
            {
                ...
                for (index_t i = 0; i < nPCoefs; ++i)
                {
                    ...
                    {
                        // For pressure, constrained DOFs should still have meaningful values
                        // They are not necessarily zero (unlike velocity Dirichlet BCs)
                        // For now, extrapolate from nearby free DOFs or use average
