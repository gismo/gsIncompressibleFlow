/** @file perpendicular_flap_fluid_precice_no_remesh.cpp
    @brief Fluid participant for a preCICE FSI example (perpendicular flap style),
           simplified to keep core FSI coupling without mesh optimization/remeshing.
...
int main(int argc, char* argv[])
{
    ...
    gsCmdLine cmd("Fluid participant (ALE, no-remesh) for perpendicular-flap FSI via preCICE");
    ...
    gsInfo << "[FSI-Fluid|NoRemesh] Geometry: " << geomPath << "\n";
    gsInfo << "[FSI-Fluid|NoRemesh] preCICE config: " << preciceConfig << "\n";

    // ---------------------- Geometry & spaces ----------------------
    gsMultiPatch<> fluidDomain;
    if (geomPath == "auto")
    {
        gsInfo << "[FSI-Fluid|NoRemesh] Building perpendicular-flap fluid domain (5 patches)...\n";
    ...
    gsInfo << "[FSI-Fluid|NoRemesh] Using interface sides: (0,east), (4,south), (1,west); externals: " << externalSides.size() << "\n";

    // ---------------------- Boundary conditions ----------------------
    gsBoundaryConditions<> bcInfo;
    // Determine bbox for classification
    real_t xmin =  std::numeric_limits<real_t>::infinity();
    real_t xmax = -std::numeric_limits<real_t>::infinity();
    real_t ymin =  std::numeric_limits<real_t>::infinity();
    real_t ymax = -std::numeric_limits<real_t>::infinity();
    ...
    // Inlet parabolic at left boundary (x = xmin). We use constant 10 m/s here for simplicity
    gsConstantFunction<> inletVec(10.0, 0.0, 2);
    gsConstantFunction<> zeroVel(0.0, 0.0, 2);

    // Tag external sides and set BCs
    const real_t tol = 1e-8;
    ...
    // NOTE: No mesh optimization/remeshing enabled here.
    ...
    while (participant.isCouplingOngoing())
    {
        ...
        // 3) Update mesh for current time (no mesh optimization)
