...
{
template <class T>
class uwbTMCSDVisitorKOmega : public uwbTMBlockVisitor<T>
{
...
public:
    uwbTMCSDVisitorKOmega(std::vector< gsDofMapper > & dofMappers, const T viscosity) :
        Base(dofMappers, viscosity), m_tauS(21), m_csdType(0), m_csdResidualType(0), m_timeStep(0.), m_alpha(1.), m_bOldSolSet(false)
    {
        m_pStabEvaluator = new uwbStabilizationEvaluator<T>();
    }

    ~uwbTMCSDVisitorKOmega()
    {
        if (m_pStabEvaluator)
        {
        ...
        gsMatrix<T>            & quNodes)
    {
        ...
        m_pStabEvaluator->setTanhCSDvars(m_csdType, m_deg, m_timeStep, m_solKOmegaGrads, residual, m_elementLength, m_alpha);
        ...
        gsVector<T> const      & quWeights)
    {
        ...
        for (index_t k = 0; k < nQuPoints; ++k) // loop over quadrature nodes
        {
            ...
            T csdStabParamK = m_pStabEvaluator->getCSDstabParam(k, 0,
            ...
            T csdStabParamO = m_pStabEvaluator->getCSDstabParam(k, 1,
    ...
    void setTanhCSD(bool unsteady = false, const T timeStep = 0., int tauS = 2, int csdType = 0, int csdResidualType = 0, T alpha = 1.)
    {
