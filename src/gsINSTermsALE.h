...
public:
                         ...
                         gsMatrix<T>& localMat) override
    {
        ...
        // The ALE convection term is: φ_i * ((u-w)·∇)φ_j
        // This is the same form as standard convection but with (u-w) instead of u
        ...
        for (index_t k = 0; k < nQuPoints; ++k)
        {
            ...
            // trialFunPhysGrad is (dim x numActive), relativeVel.col(k) is (dim x 1)
            // We want (u-w)·∇φ_j for each j, which is relativeVel^T * trialFunPhysGrad
            // Result is (1 x numActive)
            localMat.noalias() += weight * (testFunData[0].col(k) * (relativeVel.col(k).transpose() * trialFunPhysGrad));
        }
    }

    GISMO_CLONE_FUNCTION(gsINSTerm_ALEConvection)
};
