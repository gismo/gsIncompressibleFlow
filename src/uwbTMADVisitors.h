...
public:
    uwbTMADVisitorKOmega(std::vector< gsDofMapper > & dofMappers, const T viscosity) :
        Base(dofMappers, viscosity), m_r(2), m_timeStep(0.)
    {
        ...
        gsMatrix<T>            & quNodes)
    {
        ...
        m_pStabEvaluator->setADVars(m_tauStabType, m_deg, m_timeStep, m_elementLength, m_r);
    }
    ...
    void setArtificialDiffusion(const int tauType = 1, const T timeStep = 0., const int r = 2)
    {
...
public:
    uwbTMisoADVisitorKOmega(std::vector< gsDofMapper > & dofMappers, const T viscosity) :
        Base(dofMappers, viscosity), m_tauS(21), m_isoADtype(6), m_timeStep(0.), m_bOldSolSet(false)
    {
        m_pStabEvaluator = new uwbStabilizationEvaluator<T>();
    }
        ...
        gsMatrix<T>            & quNodes)
    {
        ...
        m_pStabEvaluator->initAtElement(m_solUVals);
        m_pStabEvaluator->setIsoADVars(residual, m_solKOmegaGrads, m_elementLength);
    }
    ...
    void setIsoArtificialDiffusion(bool unsteady = false, const T timeStep = 0., int tauS = 2, int isoADtype = 6)
    {
