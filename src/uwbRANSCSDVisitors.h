...
// ============================================================= PARENT ============================================================= //
template <class T>
class uwbRANSCSDVisitor : public uwbRANSBlockVisitor<T>
{
...
public:
    uwbRANSCSDVisitor(std::vector< gsDofMapper > & dofMappers, const T viscosity) :
        Base(dofMappers, viscosity), m_degU(0), m_csdType(0), m_csdResidualType(0), m_tauStabType(2),
        m_timeStep(0.), m_alpha(1.), m_r(2), m_bOldSolSet(false), m_bPSolSet(false)
    {
        ...
        gsMatrix<T>            & quNodes)
    {
        ...
        if (m_csdResidualType == 4)
            m_pStabEvaluator->setTanhCSDvars(m_csdType, m_degU, m_timeStep, solUGrads, residual, residual_adv, m_elementLength, m_alpha);
        else
            m_pStabEvaluator->setTanhCSDvars(m_csdType, m_degU, m_timeStep, solUGrads, residual, m_elementLength, m_alpha);
        ...
        gsVector<T> const      & quWeights)
    {
        ...
        for (index_t k = 0; k < nQuPoints; ++k) // loop over quadrature nodes
        {
            ...
            {
                csdStabParam = m_pStabEvaluator->getCSDstabParam(k, var);
                m_localMat[var].noalias() += weight * csdStabParam *
    ...
    void setCSD(const int csdType, const int csdResidualType, bool unsteady = false, const T timeStep = 0.)
    {
        m_csdType = csdType;
        m_csdResidualType = csdResidualType;
    ...
    void setCSD(const int csdType, const int csdResidualType, const int tauType = 1, bool unsteady = false,
